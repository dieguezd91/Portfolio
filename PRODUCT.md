...
**Education status — do not overstate.** The previous UI listed "Bachelor's Degree in Video Game Development, 2022 - 2025" at Universidad Argentina De la Empresa. Completion is **not confirmed**; that string is legacy UI copy, not a verified credential. Daniel is near the end of the programme, but completion is not yet confirmed.

**Decision: study status is not presented on the site at all.** It is not relevant to a Unity gameplay/systems positioning, and foregrounding it makes a working developer read as a student. So: no conferred degree is asserted, **no dates are shown** (any range, open or closed, reads as a completion status), **no enrolment or in-progress marker is shown**, and About does not mention studies. Education lists the field of study, the institution, and the kind of programme — nothing more. Revisit only when completion is explicitly confirmed, at which point the credential itself becomes statable. The same applies to Coderhouse (Web development) and the two Udemy courses (Game Production & Design; Unity Programming with C#): dates are held in this file as reference, not rendered. The dates on record are UADE 2022–2025, Coderhouse 2022, and both Udemy courses 2023, all from legacy UI copy.
